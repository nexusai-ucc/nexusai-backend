...
def test_should_alert(consecutive_failures, threshold, already_alerted, expected):
    assert (
        health_check_alert.should_alert(
            consecutive_failures, threshold, already_alerted
        )
        == expected
    )
...
def test_read_state_corrupt_file_returns_default(tmp_path):
    ...
    assert health_check_alert.read_state(path) == {
...
def test_send_email_logs_in_and_sends_message():
    ...
    mock_smtp.login.assert_called_once_with(
        "nexusai.alertas@gmail.com", "app-password-falsa"
    )
...
def test_send_email_failure_does_not_raise():
    ...
    with patch(
        "health_check_alert.smtplib.SMTP_SSL", side_effect=Exception("smtp unreachable")
    ):
        # No debe lanzar.
...
def test_main_healthy_resets_state(tmp_path):
    ...
    health_check_alert.write_state(
        config_path, {"consecutive_failures": 2, "alerted": False}
    )

    with patch(
        "health_check_alert._env_config",
        return_value=_config(tmp_path, state_file=config_path),
    ), patch("health_check_alert.check_health", return_value=True), patch(
        "health_check_alert.send_email"
    ) as mock_send:
        exit_code = health_check_alert.main()

    assert exit_code == 0
    assert health_check_alert.read_state(config_path) == {
        "consecutive_failures": 0,
        "alerted": False,
    }
...
def test_main_recovery_sends_notice(tmp_path):
    config_path = tmp_path / "state.json"
    health_check_alert.write_state(
        config_path, {"consecutive_failures": 5, "alerted": True}
    )
...
def test_main_failure_below_threshold_does_not_alert(tmp_path):
    ...
    with patch(
        "health_check_alert._env_config",
        return_value=_config(tmp_path, state_file=config_path, failure_threshold=3),
    ), patch("health_check_alert.check_health", return_value=False), patch(
        "health_check_alert.send_email"
    ) as mock_send:
        exit_code = health_check_alert.main()
